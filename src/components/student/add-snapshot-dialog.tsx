...
interface AddSnapshotDialogProps {
  ...
  open: boolean;
  onClose: () => void;
  lastSnapshot?: StudentState | null;
...
const selectStyle: React.CSSProperties = {
  ...inputStyle, fontFamily: "var(--font-sans)", cursor: "pointer",
};
      ...
      <div className="flex flex-wrap" style={{ gap: 4, marginBottom: values.length ? 6 : 0 }}>
        {values.map((v) => (
          <span key={v} className="inline-flex items-center gap-1"
            style={{ fontSize: 12, fontFamily: "var(--font-sans)", padding: "2px 8px", background: "var(--color-hover-bg-strong)", borderRadius: "var(--radius-xs)", color: "var(--color-text-secondary)" }}
          >
            {v}
            <button type="button" onClick={() => onChange(values.filter((x) => x !== v))}
              style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-text-muted)", lineHeight: 1, padding: 0, fontSize: 12 }}
            >×</button>
          </span>
        ))}
      </div>
...
export function AddSnapshotDialog({ studentId, open, onClose, lastSnapshot, currentGrade }: AddSnapshotDialogProps) {
      ...
      <div className="flex flex-col" style={{ gap: "var(--space-md)" }}>
        ...
        {mode === null ? (
            ...
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: "var(--space-sm)" }}>
              ...
              >
                ...
                <span style={{ fontSize: 11, fontFamily: "var(--font-sans)", color: "var(--color-text-muted)" }}>
                  Manually enter scores and preferences
                </span>
              </button>
              <div
          ...
          <>
            ...
            {step === 1 ? (
              ...
              <div className="flex flex-col" style={{ gap: "var(--space-md)" }}>
                <div>
                  ...
                  <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: "var(--space-sm)" }}>
                    <Field label="Grade"><TextInput value={grade} onChange={setGrade} placeholder="e.g. 11" /></Field>
                    <Field label="Budget (USD/year)"><NumericInput value={budget} onChange={setBudget} placeholder="e.g. 50000" /></Field>
                    <Field label="Min Acceptance Rate (%)"><NumericInput value={minAcceptance} onChange={setMinAcceptance} placeholder="e.g. 20" /></Field>
            ...
            {error && (
              <p style={{ fontSize: 13, color: "var(--color-destructive)", margin: 0 }}>{error}</p>
            )}
